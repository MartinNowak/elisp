...
in exactly one slash."
  (make-directory emacros-global-dir t)
  ...
  (catch 'hit
    (while t
      (let* ((name (read-from-minibuffer prompt "" ;; emacros-minibuffer-local-map
        ...
        (if (empty-name-p name)
            (throw 'hit nil)
          (let ((symbol (car (read-from-string name))))
            (if (and (fboundp symbol)
                ...
                (progn
                  (let ((message-log-max nil))
                    (message "%s [must not be an existing Emacs function]"
                             prompt
                             ))
                  (sit-for 3)
                  ;;(message "Function %s is already defined and not a keyboard macro" symbol)
                  nil)
              (throw 'hit symbol))))))))
;; (emacros-read-macro-name1 "Macro name: ")
...
or moved to in the current buffer."
  ...
  (let ((symbol (emacros-read-macro-name1 "Name for last kbd-macro (default to automatic name): ")))
    (when symbol
      (let ((macro-file (concat (emacros-processed-mode-name) "-emacros.el"))
            (filename)
            (gl)
            (buf)
        ...
        (if arg
            (message "Wrote definition of %s to file %s" symbol (faze filename 'file))
          (message "Wrote definition of %s to %s file %s"
                   symbol
                   (if (= gl ?g) "global" "local")
                   (faze macro-file 'file))
...
named, inserted, or manipulated macro in the current buffer."
  ...
  (let* ((old-name (emacros-read-macro-name2 "Replace macroname"))
         ...
         (macro-file (concat (emacros-processed-mode-name) "-emacros.el"))
         (filename)
...
or manipulated macro in the current buffer."
  ...
  (let ((name (emacros-read-macro-name2 "Move macro named"))
    ...
    (if (null moved)
        (error "Macro named %s not found in %s file %s"
               name (if (= gl ?l) "local" "global") (faze macro-file 'file))
      ...
      (message "Moved macro named %s to %s file %s"
               name (if (= gl ?l) "global" "local") (faze macro-file 'file)))))
...
(defun emacros-load-macros ()
  "Tries to load files mode-emacros.el
...
created during present session."
  ...
  (let ((processed-mode-name (emacros-processed-mode-name)))
    (let ((macro-file (concat processed-mode-name "-emacros.el"))
          (mac-ok)
          (nextmac)
          (filename))
      (catch 'found-mode
