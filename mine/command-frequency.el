...
arguments."
  (let* ((list (command-frequency-list reverse)) (sum (car list)))
    (mapconcat
     (cond ((not func)
            (lambda (e) (let ((sym (car e)))
                          (concat
                           (faze (format "%7d" (cdr e)) 'number)
                           "  "
                           (faze (format "%s" sym) 'function)
                           (when (commandp sym)
                             (concat
                              "  "
                              (symbol-function-keys-string sym)))
                           "\n"
                           ))))
           ((equal func t)
            (lambda (e) (let ((sym (car e)))
                          (concat
                           (faze (format "%7d" (cdr e)) 'number)
                           "  "
                           (faze (format "%6.2f%%" (/ (* 1e2 (cdr e)) sum)) 'number)
                           "  "
                           (if (symbolp sym)
                               (faze (symbol-name sym) 'function)
                             (format "%s" sym))
                           (when (commandp sym)
                             (concat
                              "  "
...
- with negative prefix argument - REVERSE was 'no-sort."
  ...
  (interactive (list nil
                     (cond
                      ...
                      (t 'no-sort))
                     nil t))
  (with-current-buffer command-frequency-buffer
  ...
  (cond ((not where)
        ...
        (t
         (let ((buffer (if (and (stringp where) (string= where ""))
...
- with negative prefix argument - REVERSE was 'no-sort."
  ...
  (with-temp-message (format "Saving frequencies to %s" (faze file 'file))
  ...
  (if (not (file-exists-p file-name))
      (progn
        (if (called-interactively-p)
            (message "File %s does not exist" (faze file-name 'file)))
        nil)
    ...
    (let ((l (with-temp-buffer
      ...
      (if (or (not (listp l)) (null l))
          (progn
            (if (called-interactively-p)
                (message "File %s does not contain any valid data" (faze file-name 'file)))
            nil)
