...
discarded. See variable `undo-limit'."
  ...
  (cond ((not (eq this-command last-command))
	 ...
	 (if (< (prefix-numeric-value arg) 0)
	     (message "Negative arg: Cannot reverse as the first operation") (ding))))
  (cond ((null buffer-undo-list)
	 (message "Buffer has not been changed") (ding))
	((eq buffer-undo-list t)
	 (message "No change info (undo is disabled)") (ding)))
    ...
    (while (null pos)
      ...
      (if (< new-probe-depth 1)
          (progn
            ...
            ;; (message "No later change info") (ding)
            )
        )
      (if (> n 150)
	  (message "working..."))
      ...
      (while (and (< n new-probe-depth)
                  (not (null l)))
	(cond ((null l)
               ;;(setq this-command t)	; Disrupt repeat sequence
               (message "No further change info")
               (ding)
