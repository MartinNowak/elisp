...
(defcustom highwin-nblines 10
  "Number of lines for overlay."
  :group 'highwin)

(defcustom highwin-timeout 0.5
...
(defun highwin-advice (&optional window nblines timeout)
  "Highlight NBLINES lines of WINDOW during TIMEOUT seconds."
  (unless window (setq window (selected-window)))
  (unless nblines (setq nblines highwin-nblines))
  (unless timeout (setq timeout highwin-timeout))
  (let ((h (/ nblines 2))
        (buf (window-buffer window)))
    (overlay-put highwin-overlay 'window window)
    ...
    ;; Disable the overlay.
    (move-overlay highwin-overlay (point-min) (point-min) buf)
    ))
