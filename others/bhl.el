...
;; Time-stamp: <2008-01-03 21:47:15 guerry>
 
;; Copyright (C) 2002 2003 2004 2005 2006 2007 2008 Bastien Guerry

;; Author: Bastien Guerry <bzg AT altern DOT org>
;; Maintainer: Bastien Guerry <bzg AT altern DOT org>
;; Version: $Id: bhl.el,v 0.4 2007/08/22 00:15:14 guerry Exp guerry $
...
;; X-URL: <http://www.cognition.ens.fr/~guerry/bhl/download/bhl.el>
...
(defcustom bhl-html-para-align "none"
  "*How to justify (align) paragraphs when converting to HTML."
  :type '(radio (const "left")
		(const "center")
		(const "justify")
		(const "right")
...
(defcustom bhl-txt-para-justify "none"
  "*How to justify paragraphs when converting to text."
  :type '(radio (const "left")
		(const "center")
		(const "full")
		(const "right")
...
(define-key bhl-mode-map [(control c) (control c) ?g] 'bhl-view-log)
	...
	(let ((tab (if (eq arg 'texi) ""
	  ...
	  (while (re-search-forward
		  ...
		  (save-match-data
		    (save-excursion
		      (forward-char 1)
		      (re-search-forward "^[\\'Å´\"&*_=a-zA-Z0-9<]" nil t))) t)
	    (replace-match
...
(defun bhl-convert-para (arg)
  ...
  (save-excursion
    ...
    (when (eq arg 'sgml)
      ...
      (end-of-line))
    (while (re-search-forward "^[\\'Å´\"&*_=a-zA-Z0-9@]" nil t)
      (goto-char (match-beginning 0))
	  ...
	  (save-match-data
	    (cond ((eq arg 'sgml)
		   (insert "<p>\n"))
		  ((eq arg 'html)
		   (if (equal bhl-html-para-align "none")
		       (insert "<p>\n")
		     (insert "\n<p align=\""
			     bhl-html-para-align "\">")))
		  ((eq arg 'txt)
		   (let ((default-justification bhl-txt-para-justify))
		     (fill-paragraph nil)))
		  (t (error "Function bhl-convert-para called with %s" arg)))
	    (forward-paragraph 1)
	    (when (memq arg '(html sgml))
	      (insert "</p>\n")))
	(forward-line 1)))))
...
(defun bhl2xxx (arg)
  "Convert BHL into one of HTML, SGML, LaTeX, txt, wiki, Texinfo.
...
mode."
  ...
  (message "Converting into %s format...done. "
	   (symbol-name arg))
  (message "View the log: M-x bhl-view-log"))
