...
(basic-set-key-alist sdcv-key-alist nil "C-z") ;sdcv的全局按键绑定
;;; ### Vi-move ###
;;; --- Vi式移动
...
(basic-set-key-alist
 '(
   ...
   ("s-U" . execute-command-with-region-replace) ;在选择的区域中执行命令并替换
   ("s-P" . execute-command-with-region-kill)    ;在选择的区域中执行命令并删除
   ("s-\"" . copy-rectangle-to-register)         ;拷贝矩形到寄存器
   ))
;;; ### Customize ###
;;; --- 自定义模式
(basic-set-key-alist doi-key-alist custom-mode-map) ;doi 的局部按键绑定
;;; ### Font ###
;;; --- 字体命令
(basic-set-key-alist
 '(
...
(scim-define-common-key (kbd "M-j") t)  ;scim切换键， 对应于SCIM中的GUI设置
...
(basic-set-key-alist
 '(
   ...
   ("M-s-o" . yaoddmuse-edit-default)   ;编辑 EmacsWiki
   ))
;;; ### Thingh-edit ###
;;; --- 增强式编辑当前光标的对象
(basic-set-key-alist
...
(basic-set-key-alist vi-move-key-alist emms-playlist-mode-map) ;vi-move 的局部按键
;;; ### Emms Tag Editor ###
;;; --- Emms 标签编辑器
(basic-set-key-alist
 '(
...
(basic-set-key-alist sdcv-key-alist emms-browser-mode-map)    ;sdcv 的局部按键
(basic-set-key-alist vi-move-key-alist emms-browser-mode-map) ;vi-move 的局部按键
;;; ### EMMS Stream ###
;;; --- EMMS 流媒体
(basic-set-key-alist
 '(
...
(basic-set-key-alist vi-move-key-alist emms-stream-mode-map) ;vi-move 的局部按键
;;; ### ECB ###
;;; --- Emacs 代码浏览器
(basic-set-key-alist
 '(
...
(basic-set-key-alist
 '(("1" . emms-play-online)                         ;在线听音乐
   ...
   ("&" . w3m-edit-emacswiki-page)                  ;编辑 emacswiki 页面
...
(basic-set-key-alist sdcv-key-alist w3m-mode-map) ;sdcv的局部按键绑定
(basic-unset-key-list
 '("s")
 w3m-mode-map)                          ;卸载按键
(basic-set-key-alist
...
(basic-set-key-alist
 '(
   ("C-z e" . auto-install-from-emacswiki) ;从emacswiki.org安装elisp文件
   ("C-z u" . auto-install-from-url)       ;从链接安装elisp文件
   ("C-z b" . auto-install-from-buffer)    ;从当前buffer安装
   ("C-z x" . auto-install-from-library)   ;更新库
   ))
;;; ### Paste2 ###
;;; --- paste2.org 的粘贴服务
(basic-set-key-alist
...
(basic-set-key-alist doi-key-alist erc-mode-map) ;doi 的局部按键
(basic-set-key-alist
 '(
   ("C-c i" . switch-to-irc)                     ;切换到IRC或自动登录IRC
...
;;; ### irfc ###
;;; --- RFC 文档阅读
(basic-set-key-alist sdcv-key-alist irfc-mode-map)
(basic-set-key-alist
 '(
...
(basic-set-key-alist sdcv-key-alist less-minor-mode-map)    ;sdcv的局部按键绑定
(basic-set-key-alist vi-move-key-alist less-minor-mode-map) ;vi-move 的局部按键
;;; ### Hexl ###
;;; --- 十六进制模式
(basic-set-key-alist
 '(
...
(basic-set-key-alist
 '(
   ("M-," . company-cycle)              ;向前切换
   ("M-." . company-cycle-backwards)    ;向后切换
   ("M-h" . company-expand-common)      ;补全公共部分
   ("M-H" . company-expand-top)         ;补全最顶端
   )
 company-mode-map
 )
;;; ### Auto-complete ###
...
(basic-set-key-alist vi-move-key-alist xgtags-select-mode-map) ;vi-move 的局部按键
(basic-set-key-alist sdcv-key-alist xgtags-select-mode-map)    ;sdcv的局部按键
(basic-set-key-alist
 '(
...
(basic-set-key-alist
 '(
   ("C-1" . multi-scratch-new)          ;新建草稿
   ("C-2" . multi-scratch-prev)         ;上一个草稿
   ("C-3" . multi-scratch-next)         ;下一个草稿
   ))
;;; ### Ido ###
;;; --- 交互式管理文件和缓存
(basic-set-key-alist
...
(basic-set-key-alist sdcv-key-alist top-mode-map)    ;sdcv 的局部按键
(basic-set-key-alist vi-move-key-alist top-mode-map) ;vi-mode的局部按键
;;; ### Color-moccur ###
;;; --- 增强的moccur
(basic-set-key-alist
 '(
...
(basic-set-key-alist sdcv-key-alist doc-view-mode-map) ;sdcv的局部按键绑定
;;; ### Gnus ###
;;; --- 新闻阅读器
(basic-unset-key-list
 '("M-K" "s")
...
(basic-set-key-alist vi-move-key-alist gnus-summary-mode-map) ;vi-move 的局部按键
(basic-unset-key-list '("t" "T") gnus-group-mode-map)         ;卸载一些按键
(basic-set-key-alist
 '(
   ("f" . gnus-group-read-group)        ;读取组
...
(basic-set-key-alist vi-move-key-alist gnus-group-mode-map) ;vi-move 的局部按键
(basic-set-key-alist
 '(
...
(basic-set-key-alist vi-move-key-alist gnus-server-mode-map) ;vi-move 的局部按键
(basic-set-key-alist
 '(
   ("J" . scroll-up-one-line)           ;向上滚动一行
   ("K" . scroll-down-one-line)         ;向下滚动一行
...
(basic-set-key-alist vi-move-key-alist gnus-browse-mode-map) ;vi-move 的局部按键
;;; ### Apt-utils ###
;;; --- Apt 管理工具
(basic-unset-key-list
 '("s")
...
(basic-set-key-alist sdcv-key-alist apt-utils-mode-map)    ;sdcv的局部按键绑定
(basic-set-key-alist vi-move-key-alist apt-utils-mode-map) ;vi-move 的局部按键
;;; ### Paredit ###
;;; --- 结构化编程
(basic-unset-key-list
...
(basic-set-key-alist
 '(
   ...
   ("C-r" . anything-select-source)     ;选择源
   )
 anything-map
 )
;;; ### Timid ###
...
(basic-set-key-alist sdcv-key-alist apropos-mode-map)    ;sdcv的局部按键绑定
(basic-set-key-alist vi-move-key-alist apropos-mode-map) ;vi-move 的局部按键
;;; ### Sys-apropos ###
;;; --- 系统相关查询
(basic-set-key-alist
 '(
...
(basic-set-key-alist sdcv-key-alist sys-apropos-mode-map)    ;sdcv 的局部按键
(basic-set-key-alist vi-move-key-alist sys-apropos-mode-map) ;vi-mode 的局部按键
;;; ### Help ###
;;; --- 帮助模式
(basic-unset-key-list
 '("e" "h" "y")
...
(basic-set-key-alist
 '(
   ("C-h h" . describe-hash)            ;解释HASH
   ("C-h u" . describe-unbound-keys)    ;查找未绑定的按键
   ("C-h p" . describe-char)            ;解释字符
   ("C-h f" . describe-function)        ;解释函数
   ("C-h C-v" . describe-face)          ;解释外观
   ("C-h C-k" . describe-key-briefly)   ;简要的解释按键
   ("C-h x" . find-function-on-key)     ;查找按键调用的函数
   ("C-h P" . finder-by-keyword)        ;根据关键字查询
   ("C-h ;" . wtf-is)                   ;查询当前光标处的缩写词
   ("C-h 7" . eyedropper-foreground)    ;取得当前光标处的前景色
   ("C-h 8" . eyedropper-background)    ;取得当前光标处的背景色
   ("C-h z" . apropos)                  ;正则表达式搜索(所有)
   ("C-h F" . apropos-function)         ;正则表达式搜索(函数)
   ("C-h V" . apropos-variable)         ;正则表达式搜索(变量)
   ("C-h O" . apropos-option)           ;正则表达式搜索(选项)
   ("C-h C" . apropos-command)          ;正则表达式搜索(命令)
   ("C-h {" . apropos-library)          ;正则表达式搜索(库)
   ("C-h \"" . apropos-value)           ;正则表达式搜索(值)
   ("C-h Z" . sys-apropos)              ;系统命令正则表达式搜索
   ("s-h b" . xray-help-buffer)         ;显示内部结构 (Buffer)
   ("s-h s" . xray-help-symbol)         ;显示内部结构 (Symbol)
   ("s-h w" . xray-help-window)         ;显示内部结构 (Window)
   ("s-h h" . xray-help-hooks)          ;显示内部结构 (Hook)
   ("s-h m" . xray-help-marker)         ;显示内部结构 (Marker)
   ("s-h o" . xray-help-overlay)        ;显示内部结构 (Overlay)
   ("s-h p" . xray-help-position)       ;显示内部结构 (Position)
   ("s-h e" . xray-help-screen)         ;显示内部结构 (Screen)
   ("s-h z" . xray-help-frame)          ;显示内部结构 (Frame)
   ("s-h x" . xray-help-features)       ;显示内部结构 (Features)
   ("s-h c" . xray-help-faces)          ;显示内部结构 (Faces)
   ))
(basic-set-key-alist
 '(
...
(basic-set-key-alist vi-move-key-alist help-mode-map) ;vi-move 的局部按键
;;; ### Info ###
;;; --- Info 模式
(basic-set-key-alist
 '(
...
(basic-set-key-alist sdcv-key-alist Info-mode-map)    ;sdcv的局部按键绑定
(basic-set-key-alist vi-move-key-alist Info-mode-map) ;vi-move 的局部按键
;;; ### Undo Browse ###
;;; --- 强大的撤销系统
(basic-set-key-alist
 '(
...
(basic-set-key-alist vi-move-key-alist ub-mode-map-default ) ;vi-move 的局部按键
;;; ### Calc ###
;;; --- 计算器
(basic-set-key-alist
 '(
...
(basic-set-key-alist sdcv-key-alist Man-mode-map)    ;sdcv的局部按键绑定
(basic-set-key-alist vi-move-key-alist Man-mode-map) ;vi-move 的局部按键
;;; ### Ielm ###
;;; --- Emacs Lisp 解释模式
(basic-unset-key-list
 '("M-p" "M-n")
...
;;; ### Completion List ###
;;; --- 补全列表
(basic-set-key-alist vi-move-key-alist completion-list-mode-map) ;vi-move的局部按键
(basic-set-key-alist sdcv-key-alist completion-list-mode-map)    ;sdcv 的局部按键
;;; ### Isearch ###
;;; --- 交互式搜索
(basic-set-key-alist
 '(
...
(basic-set-key-alist vi-move-key-alist babel-mode-map)
;;; ### Breadcrumb ###
;;; --- 书签管理导航
(basic-set-key-alist
 '(
...
(basic-set-key-alist
 '(
   ...
   ("C-7" . switch-to-messages)                  ;跳转到 *Messages* buffer
   ("C-8" . find-function-or-variable-at-point)  ;查找特定的函数或变量
   ))
;;; ### Buffer Move ###
;;; --- 缓存移动
(basic-set-key-alist
...
(basic-set-key-alist
 '(
   ...
   ("s-v b" . clean-emacs-backup)        ;清空备份文件
   ("s-&" . killall)                     ;杀掉进程
