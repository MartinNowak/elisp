...
;; jiseki-desktop - Shows the desktop so you can interact with it
;; jiseki-push - Push jiseki onto the stack, so you can check the desktop temporarily
;; jiseki-pop - pop the stack, so you can to back to whatever.

;;; TODO:
;; - Fix the volitile sources (better optimization)
;; - random thing source
;; - erc and gnus sources.
;; - make sure the point jumping around goes away.
...
(setq consed-jiseki-source
	  '((name . "Information on what has been consed.")
		(volatile)
		(display . (lambda () (format " Consed:  ():%d  0.1:%d  []:%d  ':%d  \":%d  i:%d"
									  cons-cells-consed
									  floats-consed
									  vector-cells-consed
									  symbols-consed
...
(when (featurep 'twit)
	  (setq twit-jiseki-source
			'((name . "Shows the most recent tweet.")
			  ...
			  (display . (lambda () (format "%s: %s" (cadr twit-last-tweet) (caddr twit-last-tweet)))))))
...
(defun jiseki-push ()
   ...
   (let ((jiseki-sources (append jiseki-sources (list jiseki-pop-source)))
		 (jiseki-post-display-hook (cons (lambda () (backward-char 6)) jiseki-post-display-hook)))
	 (jiseki-desktop)))
