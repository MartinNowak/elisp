...
;;; Commentary:
;;
;; This is the export engine for Org, based on previous
;; `org-export-latex.el'.
;; 
;; 
;;
		...
		(append output
			(list 
			 (list :level (or level 1)
			       ...
			       :content (org-export-parse-content)
  ...
  (save-excursion
    (let* ((props (org-combine-plists 
      ...
      (funcall (cdr (assoc 'first-lines struct-backend)) 
	       first-lines props)
      (org-export-render-main parsed-buffer props filter
...
rendering engine."
  (mapc (lambda(s)
	  ...
	  (insert 
	   (org-export-render-content s props content-backend) "\n\n")
