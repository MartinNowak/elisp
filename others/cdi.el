...
;; Version: $Id: cdi.el,v 1.86 2004/10/18 13:29:53 matt Exp $
...
(if cdi-xemacs-p
    (progn
      (defalias 'cdi-line-beginning-position 'point-at-bol)
      (defalias 'cdi-line-end-position 'point-at-eol))
  (defalias 'cdi-line-beginning-position 'line-beginning-position)
  (defalias 'cdi-line-end-position 'line-end-position))

;; Other version-dependent configuration

(defconst cdi-face-property
  (cond
   ((and (not cdi-xemacs-p)
	 (or
    ...
    (with-temp-buffer
      ...
      (if (cdi-cd-playing-or-paused-p)
          (progn
            ...
            (goto-char (point-min))
            (if (re-search-forward regexp (point-max) t)
                (setq cdi-current-track (string-to-number (match-string 1)))
  ...
  (with-temp-buffer
      ...
      (if regexp
          (progn
            ...
            (goto-char (point-min))
            (re-search-forward regexp)
            (setq cdi-current-volume (string-to-number (match-string 1)))))
      ...
      (if regexp
          (progn
            ...
            (goto-char (point-min))
            (re-search-forward regexp)
            (setq cdi-maximum-volume (string-to-number (match-string 1)))))
      ...
      (if regexp
          (progn
            ...
            (goto-char (point-min))
            (re-search-forward regexp)
            (setq cdi-minimum-volume (string-to-number (match-string 1)))))
