...
(defun pwsafe-process-filter (proc string)
  (with-current-buffer (process-buffer proc)
    (if (string-match pwsafe-password-prompt-regexp string)
      ...
      (insert string)
      (cond ((member pwsafe-running-command '(copy-user-and-passwd))
             (dolist (line (split-string string "\n"))
               (when (string-match "\\(You are ready to paste\\|Sending \\(username\\|password\\) for\\)" line)
                 (message (format "pwsafe: %s" line)))))
	    ((eq pwsafe-running-command 'copy-user)
	     (goto-char (point-min))
	     (re-search-forward "^username for [^:]*: " nil t 1)
	     (narrow-to-region (point) (line-end-position))
	     (goto-char (point-min))
	     (setq usrname (buffer-substring (point) (line-end-position)))
	     (message "usrname is %s" usrname)
	     (call-process-region (point-min) (point-max) "xsel" nil)
	     (x-select-text usrname))
	    ((eq pwsafe-running-command 'copy-passwd)
	     (goto-char (point-min))
	     (re-search-forward "^password for [^:]*: " nil t 1)
	     ;(push-mark (point) nil t)
	     ;(end-of-line)
	     (narrow-to-region (point) (line-end-position))
	     (goto-char (point-min))
	     (call-process-region (point) (line-end-position) "xsel" nil)
	     ;(kill-line)
	     ;(yank)
	     (setq passwd (buffer-substring (point) (line-end-position)))
	     (message "password is %s" passwd)
	     (x-select-text passwd))
...
(defun pwsafe-copy-user-name ()
  ...
  (let ((info (pwsafe-list-line-info)))
    (pwsafe-run 'copy-user "-u" "-f" (pwsafe-current-database-name) (car info))))
